...
func (m *MockIBlock) ByStatus(ctx context.Context, status storage.Status, limit, offset uint64, order storage0.SortOrder) ([]storage.Block, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "ByStatus", ctx, status, limit, offset, order)
	ret0, _ := ret[0].([]storage.Block)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
...
func (mr *MockIBlockMockRecorder) ByStatus(ctx, status, limit, offset, order any) *IBlockByStatusCall {
	mr.mock.ctrl.T.Helper()
	call := mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "ByStatus", reflect.TypeOf((*MockIBlock)(nil).ByStatus), ctx, status, limit, offset, order)
	return &IBlockByStatusCall{Call: call}
}
